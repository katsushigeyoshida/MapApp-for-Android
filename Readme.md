...
・写真の位置参照
 「ダウンロード」フォルダの写真の位置を参照するとエラーになるので、この場合は「本体」または「SDカード」から「Dwonload」フォルダを通して写真を選択すると写真の位置を参照することができる   

### 履歴  
2026/03/25 トレースメッセージ修正  
2026/03/18 GPSトレース表示の時トレースデータの表示を追加  
2026/02/23 GPSトレースリストのグループに複数設定(カンマ区切り)追加、ソートボタンの長押しでタイトルとコメントの検索を追加  
2026/02/14 GPSトレースの中断機能追加(長押し)とアプリダウン時の継続機能を追加　　
2026/02/10 GPSトレースリストファイルを年別に変更(ファイルの巨大化を防ぐ)  
2026/02/04 GPSトレース中断時の再開機能追加,画面レイアウト再調整  
2026/01/15 地図画面での長押しメニューのバグに対応(位置情報の受渡しミス)  
2026/01/14 地図データのダウンロードを非同期処理に変更、これによって操作不可になることを防ぐ  
　　　　　　代わりに地図の中抜けや地図の非表示が発生する場合があるが画面の更新や移動で再表示される。  
